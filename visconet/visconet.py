...
class ViscoNetLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, control_cond_config, 
                 control_crossattn_key, mask_key=None, enable_mask=True, cond_stage_img_key=None, *args, **kwargs):
        
        self.cond_stage_img_key = cond_stage_img_key
        
        super().__init__(*args, **kwargs)
        self.control_model = instantiate_from_config(control_stage_config) # refers to ControlNet
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        # Get the latent of the input images and the embedded text conditioning
        x, c_text = super().get_input(batch, self.first_stage_key, *args, **kwargs)

        # Get the openpose for controlnet
        ...
        def format_input(key):
            '''
            Receiving a key in the batch dictionary, take the elements up to batch size, convert it to contiguous format and 
            retun the tensor to the specified device
            '''
            val = batch[key]
            if bs is not None:
                val = val[:bs]
        ...
        # Get the embeddings of the fashion attributes
        if self.control_crossattn_key:
            ret_dict['c_crossattn']=[format_input(self.control_crossattn_key)]

        # Get the mask of the human subject
        if self.mask_key:
            ret_dict['c_concat_mask']=[format_input(self.mask_key)]

        # TODO: Settle this after making the IP-Adapter class
        # STEP: Get the inverse mask of human subject as image prompt for ip-adapter
        # if self.cond_stage_img_key:
        #     inverse_mask = ip_adapter.get_mask(self.mask_key)
        #     ret_dict['c_img']=[format_input(inverse_mask)]

        # Returns the latents of the source image and the dictionary of the conditions
        return x, ret_dict
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        # if no controlnet conditioning is used,
        if cond['c_concat'] is None:
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, control=None, only_mid_control=self.only_mid_control)
        else:
            # Get the output of ControlNet
            control = self.control_model(x=x_noisy, 
                                         hint=cond_concat,
                                         timesteps=t, 
                                         context=emb_cross)
            # At every level of output, we multiply the output of the controlnet with the mask
            ...
            if self.cond_stage_img_key:
                context = torch.cat((cond_txt, cond_img), dim=1)
            else:
                context = cond_txt
            eps = diffusion_model(x=x_noisy, timesteps=t, context=context, control=control, only_mid_control=self.only_mid_control)
