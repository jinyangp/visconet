...
class AppearanceEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.get_featuremaps_method == "conv":
           for mult_factor in channel_mult:
            ...
            conv_modules.append(SwinTransformer(img_size=64,
                                                ...
                                                depths=(2,2), # (num layers, num attn block per layer)
                                                num_heads=(8,16),
        ...
        for i, mult_factor in enumerate(channel_mult):
           zero_convin.extend([nn.Conv2d(model_channels*mult_factor, self.transformer_block_dim, kernel_size=1, stride=1, padding=0)])    
           zero_projout.extend([nn.Linear(self.img_dims[i]**2, self.proj_out_dim)])      
